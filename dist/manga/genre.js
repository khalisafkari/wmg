...
class Genre {
    constructor() {
        this.genreAvailable = () => __awaiter(this, void 0, void 0, function* () {
            const url = yield _1.default.get('/manga-list/');
            const $ = cheerio_1.default.load(url.data);
            const data = $('ul.genres li').map((index, item) => {
                var _a;
                return ({
                    _id: (_a = $(item).find('a').attr('href')) === null || _a === void 0 ? void 0 : _a.replace(/((http(s?))\:\/\/)westmanga.info\/genre\//g, '').replace('/', ''),
                    _title: $(item).find('a').text(),
                    _total: Number($(item).contents().eq(-1).text().replace('(', '').replace(')', ''))
                });
            }).get();
            return data;
        });
        this.postByGenre = ({ genre, page }) => __awaiter(this, void 0, void 0, function* () {
            var _a;
            const url = yield _1.default.get(`/genre/${genre ? genre : '4-koma'}/page/${page ? page : 1}/`);
            const $ = cheerio_1.default.load(url.data);
